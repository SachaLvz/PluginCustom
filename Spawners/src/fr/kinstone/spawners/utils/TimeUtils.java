...
public class TimeUtils
{
  ...
  public static String getRemaining(long duration, boolean milliseconds, boolean trail)
  {
    if ((milliseconds) && (duration < MINUTE)) {
      return 
        ((DecimalFormat)(trail ? REMAINING_SECONDS_TRAILING : REMAINING_SECONDS).get()).format(duration * 0.001D) + 's';
    }
    if (duration >= DAY) {
  ...
  public static String getUptime(long startTime)
  {
    ...
    String strTime = hours + " heures " + minutes + " minutes " + seconds + " secondes et " + totalTime % 1000L + " millisecondes.";
    return strTime;
  }
}
